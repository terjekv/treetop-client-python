...
from treetop_client.models import Action, Group, QualifiedId, Request, Resource, User
...
def test_qualified_id_and_group():
    ...
    req = Request(
        ...
        resource=Resource(kind="Photo", attrs={"id": "1"}),
    )
    api = req.to_api()
    assert api["principal"]["User"]["id"] == {"id": "alice", "namespace": ["App"]}


def test_resource_empty_attrs():
    with pytest.raises(ValueError):
        Resource(kind="Photo", attrs={})
...
def test_action_with_namespace():
    ...
    a = Action(id=q)
    assert a.to_api() == {"id": {"id": "edit", "namespace": ["App"]}}
